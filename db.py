...
def get_client() -> MongoClient:
    ...
    IMPORTANTE: esta función NO atrapa excepciones a propósito. Si se
    atraparan aquí y se retornara `None`, `st.cache_resource` cachearía
    ese `None` como si fuera un resultado válido, y la app quedaría con
    la conexión "rota" hasta un reinicio manual — incluso después de
    arreglar el secreto o que vuelva la red. Dejando que la excepción
    se propague, Streamlit NO cachea el fallo y cada llamada reintenta
...
def get_db():
    """
    Devuelve el objeto de base de datos `spbi`, o `None` si no hay
    conexión. Aquí sí se atrapa la excepción de `get_client()` (esta
    función no está cacheada, así que no hay riesgo de "congelar" un
    fallo).
    """
    try:
        client = get_client()
...
def entrenar_modelo_svc(ticker: str, cv_folds: int = CV_FOLDS_CLOUD) -> Dict[str, Any]:
    ...
    try:
        ...
        grid = GridSearchCV(
            ...
            n_jobs=2, refit=True,  # limitado (no -1) para no saturar memoria en el contenedor
        )
...
def entrenar_modelo_rnn(ticker: str, arquitectura: str, epochs: int = EPOCHS_CLOUD) -> Dict[str, Any]:
    ...
    try:
        ...
        # Reduce el footprint de memoria/hilos de TensorFlow — importante en
        # el tier gratuito de Streamlit Community Cloud (límite de 1GB RAM),
        # donde TF con su threading por defecto puede provocar un
        # Segmentation Fault por falta de memoria durante la inicialización.
        ...
        try:
            ...
            pass  # ya se configuró en una llamada anterior dentro del mismo proceso
        ...
        # Libera la sesión de Keras y fuerza garbage collection SIEMPRE
        # (éxito o error), para que la memoria de este modelo no se acumule
        # antes de que la Consola de Modelos entrene el siguiente en la
        # misma ejecución del pipeline — clave para no exceder el límite de
        # 1GB RAM del tier gratuito de Streamlit Community Cloud.
        try:
            import gc
